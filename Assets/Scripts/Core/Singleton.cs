...
public class Singleton<T> : MonoBehaviour where T : Component
{
    ...
    public static T Instance
    {
        get
        {
            if (isShutdown) // 종료 처리에 들어갔으면
            {
                Debug.LogWarning("싱글톤은 이미 삭제중이다."); // 경고 출력하고
                return null; // null 리턴
            }
            ...
            if (instance == null) // 객체가 없으면
            {
                ...
                instance = singleton; // 다른 게임 오브젝트에 있는 싱글톤이나 새로 만든 싱글톤을 저장
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        if (mode != LoadSceneMode.Additive) // additive가 아닐 때만 실행
        {
            OnInitialize();
        }
    }
...
public class TestSingleton
{
    ...
    private TestSingleton() // 객체가 중복으로 생성되는 것을 방지하기 위해 생성자를 private으로 한다. (기본 생성자가 만들어지는 것을 방지)
    {
